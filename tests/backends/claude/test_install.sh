...
test_install_claude_hooks() {
  nvim_exec "require('claude-preview.hooks').install()"
  sleep 0.3
  ...
  assert_contains "$content" "claude-preview-diff.sh" "should reference diff script" || return 1
  assert_contains "$content" "claude-close-diff.sh" "should reference close script" || return 1
...
test_uninstall_claude_hooks() {
  ...
  # Then uninstall
  nvim_exec "require('claude-preview.hooks').uninstall()"
  sleep 0.2
  ...
  assert_not_contains "$content" "claude-preview-diff.sh" "diff script should be removed" || return 1
  assert_not_contains "$content" "claude-close-diff.sh" "close script should be removed" || return 1
}
...
test_install_idempotent() {
  ...
  local count
  count="$(echo "$content" | grep -o "claude-preview-diff.sh" | wc -l | tr -d ' ')"
...
JSON
  ...
  assert_contains "$content" "claude-preview-diff.sh" "our hooks should be added" || return 1
}
